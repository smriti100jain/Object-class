#This Code find the optimised parameter for MULTI CLASSIFICATION  by 10 fold cross-validation 
...
def my_custom_loss_func(ground_truth, predictions):
			...
			C=4
            Nc=np.array([np.sum(ground_truth==1),np.sum(ground_truth==2),np.sum(ground_truth==3),np.sum(ground_truth==4)])
            ...
            for j in range(0,len(predictions)):
                ...
                if(arr!=ground_truth[j]):
                    error=error+(1/float(Nc[(ground_truth[j]-1)]))
            error=error/float(C)
...
#Rndomized Search over parameters with BER as the score function 10 fold CV
